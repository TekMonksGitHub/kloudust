...
if ! sudo nft add rule bridge "${BR_NAME}_filter" forward ether saddr "$MAC_ADDRESS" ct state established,related counter accept comment \"$RULESET_NAME-$VM_NAME-$VNET_ID\"; then exitFailed; fi
...
while read -r rule; do
    ...
    if ! sudo nft add rule bridge "${BR_NAME}_filter" forward \
        $MAC_MATCH $CT_MATCH $IP_MATCH $PORT_MATCH counter $ACTION comment \"$RULESET_NAME-$VM_NAME-$VNET_ID\"; then
        exitFailed
    fi
