...
if ! sudo nft add rule inet "ip_${IP_ADDRESS_DASHED}_filter" forward ip saddr "$PUBLIC_IP" ct state established,related counter accept comment \"$RULESET_NAME-$VM_NAME-$IP_ADDRESS_DASHED\"; then exitFailed; fi
...
while read -r rule; do
    ...
    if ! sudo nft add rule inet "ip_${IP_ADDRESS_DASHED}_filter" forward \
        $IP_MATCH $CT_MATCH $PORT_MATCH counter $ACTION comment \"$RULESET_NAME-$VM_NAME-$IP_ADDRESS_DASHED\"; then
        exitFailed
    fi
