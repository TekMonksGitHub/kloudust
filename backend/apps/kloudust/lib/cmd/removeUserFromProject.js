/** 
 * removeUserFromProject.js - removes the given user from the given project. Users
 * for a project or org admins can remove others from the project.
 * 
...
module.exports.exec = async function(params) {
    ...
    if (!await dbAbstractor.checkUserBelongsToProject(email, project, org)) {
        const warn = `User ${email} does not belong to the project ${project}`; params.consoleHandlers.LOGWARN(warn);
        return CMD_CONSTANTS.TRUE_RESULT(undefined, warn);
    } else return {result: await dbAbstractor.removeUserFromProject(email, project, org), err: "", out: ""};
}
