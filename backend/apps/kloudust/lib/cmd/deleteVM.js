...
async function _cleanupVMResources(vm) {
    ...
    for (const vmVnet of vmVnets.vnets) {
        ...
        if(vmVnetFirewall.length !== 0){
            for (const vmvnetfw of vmVnetFirewall) {
                ...
                if (!remove_result.result) { params.consoleHandlers.LOGERROR(`Firewall ruleset ${ruleset.name} could not be removed from ${vnetName} of VM ${vm.name_raw}`); return {...remove_result, ...CMD_CONSTANTS.FALSE_RESULT()}; }                
            }
        }
    }
    ...
    if (vm.ips.trim() !== "") { // Public IP firewalls use IPs not ethernets. This deletes public firewalls
