...
module.exports.exec = async function (params) {
    ...
    const isVxlanSetup = await setupVxlanIfNeeded(params,vmHostname, vlanDetails);
    if (!isVxlanSetup) {
...
async function setupVxlanIfNeeded(params,vmHostname, vlanDetails) {
    ...
    if (!(await executeXforge(params,primaryHost, secondaryHost.hostaddress, vxlanId,primaryHostLocalIp))) return false;
    if (!(await executeXforge(params,secondaryHost, primaryHost.hostaddress, vxlanId,secondaryHostLocalIp))) 
    {
        const deleteResult = await deleteVxlan(params,primaryHost,vxlanId);
        if(deleteResult) {
...
async function deleteVxlan(params,targetHost, vxlanId) {
    const xforgeArgs = {
...
async function executeXforge(params,targetHost, otherHost, vxlanId,localIp) {
    const xforgeArgs = {
        colors: KLOUD_CONSTANTS.COLORED_OUT,
        file: `${KLOUD_CONSTANTS.LIBDIR}/3p/xforge/samples/remoteCmd.xf.js`,
        console: params.consoleHandlers,
