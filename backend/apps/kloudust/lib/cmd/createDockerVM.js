...
module.exports.exec = async function (dockerFile, port, params, dockerImageName, dockerBuild) {
    ...
    let { VM_NAME, VM_DESC, VM_CORE, VM_MEMORY, VM_DISK, VM_IMG, VM_RUNCMD } = KLOUD_CONSTANTS.CONF.DOCKER_VM_PARAM;
...
const _portReachable = async (params, hostInfo) => {
    const xforgeArgs = {
        ...
        other: [
            hostInfo.hostaddress, hostInfo.rootid, hostInfo.rootpw, hostInfo.hostkey, hostInfo.port,
            `${KLOUD_CONSTANTS.LIBDIR}/cmd/scripts/portReachable.sh`, params.vm_name, KLOUD_CONSTANTS.CONF.PYTHON_HTTP_SERVER_PORT, KLOUD_CONSTANTS.CONF.S3STORAGE_DEFAULT_PORT
        ]
    }
    const results = await xforge(xforgeArgs);
    return results;
