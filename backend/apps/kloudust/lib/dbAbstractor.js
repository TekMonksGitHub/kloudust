...
exports.getHostIps = async (hostname, project=KLOUD_CONSTANTS.env.prj, org=KLOUD_CONSTANTS.env.org) => {
    if (!roleman.checkAccess(roleman.ACTIONS.lookup_project_resource)) {_logUnauthorized(); return false; }
    project = roleman.getNormalizedProject(project); org = roleman.getNormalizedOrg(org);

    results = await _db().getQuery("select internalip,externalip from hostipmapping where host = ? collate nocase", [hostname]);
    if ((!results) || (!results.length)) return null;
...
exports.getLocalIp = async (hostname,project = KLOUD_CONSTANTS.env.prj, org = KLOUD_CONSTANTS.env.org) => {
    ...
    results = await _db().getQuery("SELECT internalip  FROM hostipmapping where host is ?",[hostname]);
    if ((!results) || (!results.length)) return null;
